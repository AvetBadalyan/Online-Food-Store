...
const PAYMENT_METHODS = [
  ...
  { id: 'card', icon: '💳', name: 'Card on Delivery', desc: 'Card payment at the door' },
]
...
export default function CheckoutPage() {
  ...
  if (items.length === 0 && !orderId) {
    return (
      <div className="container">
        <div className={styles.page} style={{ textAlign: 'center', paddingTop: 'var(--space-20)' }}>
  ...
  async function goNext() {
    ...
    if (step === 1) {
      const valid = await trigger(['fullName', 'email', 'phone', 'street', 'city', 'postalCode', 'country'])
      if (!valid) return
    }
  ...
  async function onSubmit(data) {
    ...
    try {
      ...
      await postOrder({
        ...
        address: { street: data.street, city: data.city, postalCode: data.postalCode, country: data.country },
        notes: data.notes ?? '',
        paymentMethod,
        items,
      ...
      <div className="container">
        ...
        >
          ...
          <p className={styles.successText}>
            Thank you for your order. We're preparing your food and will have
            it delivered to you shortly.
          </p>
          ...
          <div style={{ display: 'flex', gap: 'var(--space-3)', marginTop: 'var(--space-2)', flexWrap: 'wrap', justifyContent: 'center' }}>
            <Link to="/" style={{ color: 'var(--color-primary)', fontWeight: 'var(--font-semi)' }}>
              Back to Menu
            </Link>
            {user && (
              <Link to="/profile" style={{ color: 'var(--color-text-muted)' }}>
                View My Orders
              </Link>
            )}
          </div>
              ...
              <AnimatePresence mode="wait">
                ...
                {step === 1 && (
                  ...
                  >
                    ...
                    <div className={styles.formActions}>
                      <Link to="/" style={{ color: 'var(--color-text-muted)', fontSize: 'var(--text-sm)', display: 'flex', alignItems: 'center', gap: 'var(--space-1)' }}>
                        <FiArrowLeft size={14} /> Back to Menu
                      </Link>
                      <button
                        type="button"
                        onClick={goNext}
                        style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)', padding: 'var(--space-3) var(--space-6)', background: 'var(--color-primary)', color: '#fff', borderRadius: 'var(--radius-md)', fontWeight: 'var(--font-semi)', fontSize: 'var(--text-sm)', transition: 'background var(--transition-base)' }}
                      >
                ...
                {step === 2 && (
                  ...
                  >
                    ...
                    <div className={styles.formActions}>
                      <button type="button" onClick={goBack} style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-1)', color: 'var(--color-text-muted)', fontSize: 'var(--text-sm)' }}>
                        <FiArrowLeft size={14} /> Back
                      </button>
                      <button
                        type="button"
                        onClick={goNext}
                        style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)', padding: 'var(--space-3) var(--space-6)', background: 'var(--color-primary)', color: '#fff', borderRadius: 'var(--radius-md)', fontWeight: 'var(--font-semi)', fontSize: 'var(--text-sm)' }}
                      >
                  ...
                  >
                    ...
                    <div className={styles.formActions}>
                      ...
                      <button
                        type="submit"
                        disabled={submitting}
                        style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)', padding: 'var(--space-3) var(--space-8)', background: 'var(--color-primary)', color: '#fff', borderRadius: 'var(--radius-md)', fontWeight: 'var(--font-semi)', fontSize: 'var(--text-sm)', opacity: submitting ? 0.6 : 1, cursor: submitting ? 'not-allowed' : 'pointer' }}
                      >
                        {submitting ? 'Placing Order…' : 'Place Order'} {!submitting && <FiCheck size={14} />}
                      </button>
                    </div>
                  </motion.div>
                )}
